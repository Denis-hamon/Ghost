...
Provenance: vendored from the Latent Imagination measurement project
(github.com/Denis-hamon/latent-imagination, scripts/mcp/) at commit 65afcf3,
adapted for standalone serving (paths env-overridable; pool + calibration
artifacts shipped under data/ and artifacts/). Apache-2.0.
...
CALIBRATION_PATH = Path(os.environ.get(
    "LI_CALIBRATION",
    ROOT / "artifacts" / "predictor-mcp-calibration.json"))
LOG_PATH = Path(os.environ.get("LI_LOG_PATH", ROOT / "data" / "mcp-log.jsonl"))
# pool servi : v8 (n=207) par défaut — surchargable par env pour tests/reculs
POOL_JSON = Path(os.environ.get(
    "LI_POOL_JSON",
    ROOT / "data" / "latent-pool-v8.json"))
POOL_NPZ = Path(os.environ.get(
    "LI_POOL_NPZ",
    ROOT / "data" / "latent-pool-v8.npz"))
...
RISK_CALIB = Path(os.environ.get(
    "LI_RISK_CALIB",
    ROOT / "artifacts" / "risk-scan-v8-calibration.json"))
